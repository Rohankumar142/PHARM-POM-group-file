# DATABASE+UI.py
# Fully modernized Pharmacy LED app using CustomTkinter
# -----------------------------------------------------
# Features:
# - Patients table (search, add, delete), double-click to open patient popup
# - Patient popup: edit name/address, add/edit prescriptions, auto-assign (with family-bin),
#   confirm/deny + manual assign, LED blink (pause/resume/done)
# - Shelf Assignment (A..Z + Overflow), Save All & Close (single button)
# - Overdue tab (list + per-patient detail) + Light Up buttons
# - Previous Actions tab (last 300 actions today)
# - Multiple locations per patient shown on main table
# - Persistent SQLite DB next to this script
# - Delete prescriptions (multi-select) via button + Delete/Backspace
# - Clear all prescriptions for a patient (frees slots)
# UI upgrades:
# - CustomTkinter dark/light modern UI (rounded buttons, color accents)
# - Bigger fonts: labels=17, table text=16, headers=20
# - Popups + messageboxes stay on top (messageboxes use parent=...)
...
def db_connect():
    # slight timeout to reduce "database locked" on fast double-clicks
...
# =====================================================
# LED simulation (simulated print-based blinker)
# =====================================================
...
def open_led_popup(parent, slots, color="yellow", title="LED Control"):
    if not slots:
        messagebox.showinfo("LED", "No slots to blink.", parent=parent); return
    key = start_blink(slots, color)
    win = ctk.CTkToplevel(parent); win.title(title)
    win.geometry("520x160"); win.attributes("-topmost", True); win.lift()
...
def style_treeview():
    ...
    style.configure("Treeview",
                    ...
                    rowheight=32,                 # larger rows
                    borderwidth=0,
                    font=("Segoe UI", 16))       # table text size
    style.configure("Treeview.Heading",
                    background="#0078d4",
                    foreground="white",
                    font=("Segoe UI Semibold", 20))  # header size
...
class App(ctk.CTk):
    def __init__(self):
        ...
        search_entry = ctk.CTkEntry(controls, textvariable=self.search_var, width=260, font=ctk.CTkFont(size=16))
        ...
        for c in ("Name","Address","Date Added","Locations"):
            ...
            self.tree.column(c, width=240 if c!="Locations" else 460, anchor="w")
    ...
    def refresh_patient_table(self):
        ...
        for i,(pid,name,addr,created) in enumerate(rows):
            ...
            # all distinct locations for this patient
    ...
    def add_patient_popup(self):
        p = ctk.CTkToplevel(self); p.title("Add Patient"); p.geometry("520x220"); p.attributes("-topmost", True); p.lift()
        ...
        name_e = ctk.CTkEntry(body, width=300, font=ctk.CTkFont(size=16)); name_e.grid(row=0, column=1, padx=8, pady=8)
        ctk.CTkLabel(body, text="Address", font=ctk.CTkFont(size=17)).grid(row=1, column=0, sticky="e", padx=8, pady=8)
        addr_e = ctk.CTkEntry(body, width=300, font=ctk.CTkFont(size=16)); addr_e.grid(row=1, column=1, padx=8, pady=8)
        def save():
            nm = name_e.get().strip()
            if not nm:
                messagebox.showerror("Error","Name required", parent=p); return
        ...
        ctk.CTkButton(body, text="Save", fg_color="#2F8B2F", hover_color="#277327", command=save).grid(row=2, column=0, columnspan=2, pady=10)
    ...
    def delete_selected_patient(self):
        ...
        if not messagebox.askyesno("Confirm", f"Delete {name} and ALL prescriptions?", parent=self): return
    ...
    def open_patient_popup(self, pid):
        ...
        win.geometry("1160x780"); win.attributes("-topmost", True); win.lift()
        ...
        name_e = ctk.CTkEntry(info, width=320, font=ctk.CTkFont(size=16)); name_e.grid(row=0, column=1, padx=6, pady=6); name_e.insert(0, pname)
        ctk.CTkLabel(info, text="Address", font=ctk.CTkFont(size=17)).grid(row=0, column=2, sticky="e", padx=6, pady=6)
        addr_e = ctk.CTkEntry(info, width=320, font=ctk.CTkFont(size=16)); addr_e.grid(row=0, column=3, padx=6, pady=6); addr_e.insert(0, paddr)
        ...
        def delete_selected_prescriptions():
            sel = rx.selection()
            if not sel:
                messagebox.showinfo("Delete", "No prescription selected.", parent=win); return
        ...
        med_e = ctk.CTkEntry(addf, width=220, font=ctk.CTkFont(size=16)); med_e.grid(row=0, column=1, padx=6)
        ctk.CTkLabel(addf, text="Quantity", font=ctk.CTkFont(size=17)).grid(row=0, column=2, sticky="e", padx=6)
        qty_e = ctk.CTkEntry(addf, width=100, font=ctk.CTkFont(size=16)); qty_e.grid(row=0, column=3, padx=6)
        ctk.CTkLabel(addf, text="Basket", font=ctk.CTkFont(size=17)).grid(row=0, column=4, sticky="e", padx=6)
        basket_cb = ctk.CTkComboBox(addf, values=["small","large"], width=120, font=ctk.CTkFont(size=16))
    ...
    def _light_up_patient(self, pid, parent):
        ...
        if not slots:
            messagebox.showinfo("LED","No assigned locations for this patient.", parent=parent); return
        open_led_popup(parent, slots, "blue", "Patient Locations")

    def _clear_all_prescriptions(self, pid, popup):
        name = db_fetchone("SELECT name FROM patients WHERE id=?", (pid,))[0]
        if not messagebox.askyesno("Confirm", f"Clear ALL prescriptions for {name}?", parent=popup): return
        rows = db_fetchall("SELECT slot_id FROM prescriptions WHERE patient_id=? AND slot_id IS NOT NULL", (pid,))
        sids = [r[0] for r in rows if r and r[0]]
        if sids: mark_slots_free(sids)
        db_exec("DELETE FROM prescriptions WHERE patient_id=?", (pid,))
    ...
    def _add_prescription(self, pid, addr_e, med_e, qty_e, basket_cb, parent, refresh_cb):
        ...
        if not med or not qty or basket not in ("small","large"):
            messagebox.showerror("Error","Enter medication, quantity, and basket size.", parent=parent); return
        ...
        def auto_assign():
            ...
            conf = ctk.CTkToplevel(parent); conf.title("Confirm Slot"); conf.geometry("520x160"); conf.attributes("-topmost", True); conf.lift()
        ...
        if fam:
            ...
            fp = ctk.CTkToplevel(parent); fp.title("Family Match"); fp.geometry("540x160"); fp.attributes("-topmost", True); fp.lift()
    ...
    def _manual_assign_new(self, parent, pres_id, refresh_cb):
        m = ctk.CTkToplevel(parent); m.title("Manual Slot Assignment"); m.geometry("420x260"); m.attributes("-topmost", True); m.lift()
        ctk.CTkLabel(m, text="Shelf (F/L/R)", font=ctk.CTkFont(size=17)).grid(row=0, column=0, padx=8, pady=8, sticky="e")
        s_e = ctk.CTkEntry(m, width=80, font=ctk.CTkFont(size=16)); s_e.grid(row=0, column=1, padx=8, pady=8)
        ctk.CTkLabel(m, text="Row (A–Z)", font=ctk.CTkFont(size=17)).grid(row=1, column=0, padx=8, pady=8, sticky="e")
        r_e = ctk.CTkEntry(m, width=80, font=ctk.CTkFont(size=16)); r_e.grid(row=1, column=1, padx=8, pady=8)
        ctk.CTkLabel(m, text="Column (1–100)", font=ctk.CTkFont(size=17)).grid(row=2, column=0, padx=8, pady=8, sticky="e")
        c_e = ctk.CTkEntry(m, width=100, font=ctk.CTkFont(size=16)); c_e.grid(row=2, column=1, padx=8, pady=8)
        def assign():
            s = s_e.get().strip().upper()
            row = r_e.get().strip().upper()
            try: col = int(c_e.get().strip())
            except:
                messagebox.showerror("Error","Invalid column", parent=m); return
            rec = get_slot_by_position(s, row, col)
            if not rec: messagebox.showerror("Error","Slot does not exist", parent=m); return
            slot_id, occ = rec
            if occ: messagebox.showerror("Error","Slot already occupied", parent=m); return
    ...
    def _edit_prescription_popup(self, patient_id, pres_id, refresh_cb):
        e = ctk.CTkToplevel(self); e.title("Edit Prescription"); e.geometry("640x500"); e.attributes("-topmost", True); e.lift()
        ...
        med_e = ctk.CTkEntry(body, width=280, font=ctk.CTkFont(size=16)); med_e.grid(row=0, column=1, padx=6, pady=6); med_e.insert(0, med)
        ctk.CTkLabel(body, text="Quantity", font=ctk.CTkFont(size=17)).grid(row=1, column=0, sticky="e", padx=6, pady=6)
        qty_e = ctk.CTkEntry(body, width=140, font=ctk.CTkFont(size=16)); qty_e.grid(row=1, column=1, padx=6, pady=6); qty_e.insert(0, str(qty) if qty is not None else "")
        ctk.CTkLabel(body, text="Date (MM/DD/YYYY)", font=ctk.CTkFont(size=17)).grid(row=2, column=0, sticky="e", padx=6, pady=6)
        date_e = ctk.CTkEntry(body, width=180, font=ctk.CTkFont(size=16)); date_e.grid(row=2, column=1, padx=6, pady=6)
        if dt:
            for fmt in ("%Y-%m-%d %H:%M:%S","%Y-%m-%d"):
        ...
        b_cb = ctk.CTkComboBox(body, values=["small","large"], width=140, font=ctk.CTkFont(size=16))
        ...
        loc_e = ctk.CTkEntry(body, width=180, font=ctk.CTkFont(size=16)); loc_e.grid(row=4, column=1, padx=6, pady=6); loc_e.insert(0, slot_id_to_label(slot_id) if slot_id else "")
        ...
        def save_edit():
            ...
            if dtxt:
                ...
                except: messagebox.showerror("Error","Date must be MM/DD/YYYY", parent=e); return
            else:
                d = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            new_loc = loc_e.get().strip()
            if new_loc and (parse_location_label_to_slot_id(new_loc) is None):
                messagebox.showerror("Error","Invalid LED location. Use F-A61 or numeric id.", parent=e); return
            new_sid = parse_location_label_to_slot_id(new_loc) if new_loc else None
            if new_sid and new_sid != slot_id:
                if not messagebox.askyesno("Confirm","Manually change LED slot?", parent=e): return
                r = db_fetchone("SELECT occupied FROM slots WHERE id=?", (new_sid,))
                if not r: messagebox.showerror("Error","Slot not found", parent=e); return
                if r[0]==1: messagebox.showerror("Error","Slot already occupied", parent=e); return
    ...
    def open_shelf_assignment(self):
        win = ctk.CTkToplevel(self); win.title("Shelf Assignment"); win.geometry("920x860"); win.attributes("-topmost", True); win.lift()
        ...
        ctk.CTkLabel(win, text="Assign ranges per letter. Bounds like A1 and Z13. Overflow is a letter section too.", font=ctk.CTkFont(size=17)).pack(anchor="w", padx=12, pady=(8,6))
        ...
        for i, htxt in enumerate(("Letter","Shelf (F/L/R)","Lower (A1)","Upper (Z13)")):
            ctk.CTkLabel(hrow, text=htxt, font=ctk.CTkFont(size=20, weight="bold")).grid(row=0, column=i, padx=10, pady=6, sticky="w")
        ...
        for i,L in enumerate(letters, start=1):
            ...
            shelf_cb = ctk.CTkComboBox(rowf, values=["","F","L","R"], width=80, font=ctk.CTkFont(size=16)); shelf_cb.set("")
            lower_e  = ctk.CTkEntry(rowf, width=120, font=ctk.CTkFont(size=16))
            upper_e  = ctk.CTkEntry(rowf, width=120, font=ctk.CTkFont(size=16))
        ...
        def save_all():
            for L,(s,lo,up) in self.letter_widgets.items():
                ...
                if S and S not in ("F","L","R"):
                    messagebox.showerror("Error", f"Invalid shelf for {L}", parent=win); return
                if LO and not parse_bound(LO):
                    messagebox.showerror("Error", f"Lower bound invalid for {L}: {LO}", parent=win); return
                if UP and not parse_bound(UP):
                    messagebox.showerror("Error", f"Upper bound invalid for {L}: {UP}", parent=win); return
            ...
            win.destroy()
        ctk.CTkButton(foot, text="💾 Save All & Close", fg_color="#0B5CAB", hover_color="#084b8a", command=save_all).pack(side="right")
    ...
    def open_overdue_tab(self):
        ...
        win = ctk.CTkToplevel(self); win.title("Overdue Medications"); win.geometry("1120x780"); win.attributes("-topmost", True); win.lift()
    ...
    def _open_overdue_patient_detail(self, tree):
        ...
        if not items:
            messagebox.showinfo("Overdue","No overdue items (refresh?)", parent=self); return
        win = ctk.CTkToplevel(self); win.title("Overdue Details"); win.geometry("900x520"); win.attributes("-topmost", True); win.lift()
        head = ctk.CTkFrame(win, fg_color="#c50f1f"); head.pack(fill="x")
    ...
    def _light_up_overdue(self, items, parent):
        ...
        if not slots:
            messagebox.showinfo("LED","No overdue items with assigned slots.", parent=parent); return
        open_led_popup(parent, slots, "red", "Overdue — LED")

    # ---------------- Previous Actions ----------------
    def open_actions_tab(self):
        win = ctk.CTkToplevel(self); win.title("Previous Actions"); win.geometry("1020x660"); win.attributes("-topmost", True); win.lift()
