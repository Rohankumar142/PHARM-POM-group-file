...
def repair_slot_occupancy():
    """
    Canonicalize occupancy: set all to 0, then mark occupied for every prescription that has a slot_id.
    This prevents 'phantom full' rows where slots stayed marked occupied after edits/deletes.
    """
    with db_connect() as conn:
        cur = conn.cursor()
        cur.execute("UPDATE slots SET occupied=0")
        cur.execute("SELECT slot_id FROM prescriptions WHERE slot_id IS NOT NULL")
...
def confirm_auto_assign_popup(parent, presc_id, slot_ids, shelf_name, refresh_cb):
    ...
    Works with the rest of the code that expects a `refresh_cb` callback.
    """
    labels = slot_ids_to_labels(slot_ids)
    short = " & ".join([l.split("-")[1] for l in labels])
    win = ctk.CTkToplevel(parent)
    win.title("Confirm Slot")
    ...
    def accept():
        ...
        # Save only the first slot id in prescriptions table (for backwards compatibility)
...
def clear_all_prescriptions_with_led(parent, patient_id, on_done_refresh):
    ...
    if not slots:
        if messagebox.askyesno("Confirm", "No slots assigned. Clear all prescriptions anyway?"):
            ...
            # Repair occupancy after clearing
            repair_slot_occupancy()
            on_done_refresh()
        return
    ...
    def confirm():
        ...
        # After major changes, repair occupancy to be safe
        repair_slot_occupancy()
        win.destroy()
        on_done_refresh()
    ...
    def open_patient_popup(self, pid):
        ...
        def delete_selected_prescriptions():
            ...
            for item in sel:
                ...
                if not row: continue
                med, sid = row
                if sid: mark_slots_free([sid])   # free the slot so auto-assign sees it
                db_exec("DELETE FROM prescriptions WHERE id=?", (prid,))
                log_action(f"Deleted prescription '{med}' for patient_id={pid}")
            populate_rx()
            self.refresh_patient_table()
