# pharmacy_app.py
# Pharmacy LED System — CustomTkinter full app
# - Fonts: labels/entries/buttons 17pt, table rows 16pt, headers 20pt bold
# - Popups always-on-top
# - Features: Patients, Rx add/edit/delete, auto-assign + family-bin + manual assign, LED blink popup
#             Shelf Assignment (A..Z + Overflow) save-all, Overdue with LED, Previous Actions,
#             Clear All Prescriptions, multiple locations per patient, search & refresh
...
def open_led_popup(parent, slots, color="yellow", title="LED Control"):
    ...
    win = ctk.CTkToplevel(parent); win.title(title); win.geometry("600x190")
...
def init_db(reset=False):
    with db_connect() as conn:
        ...
        conn.commit()
    # seed letter rows + some defaults if empty
...
def style_treeview():
    ...
    style.configure("Treeview",
                    ...
                    rowheight=34,
                    borderwidth=0,
                    font=("Segoe UI", 16))  # table text
    style.configure("Treeview.Heading",
                    background="#0078d4",
                    foreground="white",
                    font=("Segoe UI Semibold", 20))  # headers
...
class App(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkButton(topbar, text="🌓 Theme", width=100,
                      font=ctk.CTkFont(size=17),
                      command=toggle_theme).pack(side="right", padx=10, pady=8)

        # ---------- Right sidebar ----------
        sidebar = ctk.CTkFrame(self, width=240, corner_radius=0)
        sidebar.pack(side="right", fill="y")
        ctk.CTkLabel(sidebar, text="Tools",
                     font=ctk.CTkFont(size=20, weight="bold")).pack(pady=(12, 8))
        ctk.CTkButton(sidebar, text="📚 Shelf Assignment", font=ctk.CTkFont(size=17),
                      command=self.open_shelf_assignment).pack(padx=10, pady=6, fill="x")
        ctk.CTkButton(sidebar, text="⏰ Overdue Meds", font=ctk.CTkFont(size=17),
                      command=self.open_overdue_tab).pack(padx=10, pady=6, fill="x")
        ctk.CTkButton(sidebar, text="📝 Previous Actions", font=ctk.CTkFont(size=17),
                      command=self.open_actions_tab).pack(padx=10, pady=6, fill="x")

        # ---------- Patients header + controls ----------
        header = ctk.CTkFrame(self, fg_color="#0b5cab")
        header.pack(fill="x", padx=0, pady=(0,6))
        ctk.CTkLabel(header, text="Patients",
                     font=ctk.CTkFont(size=20, weight="bold")).pack(anchor="w", padx=12, pady=8)
        ...
        ctk.CTkButton(controls, text="🔄 Refresh", font=ctk.CTkFont(size=17),
                      command=self.refresh_patient_table).pack(side="left", padx=6)
        ctk.CTkButton(controls, text="➕ Add Patient",
                      font=ctk.CTkFont(size=17),
                      fg_color="#2F8B2F", hover_color="#277327",
                      command=self.add_patient_popup).pack(side="right", padx=6)
        ctk.CTkButton(controls, text="🗑️ Delete Patient",
                      font=ctk.CTkFont(size=17),
                      fg_color="#D83B01", hover_color="#B32F00",
    ...
    def add_patient_popup(self):
        ...
        ctk.CTkLabel(head, text="Add Patient",
                     font=ctk.CTkFont(size=20, weight="bold")).pack(anchor="w", padx=12, pady=8)
        ...
        name_e = ctk.CTkEntry(body, width=360, font=ctk.CTkFont(size=17)); name_e.grid(row=0, column=1, padx=8, pady=8)
        ctk.CTkLabel(body, text="Address", font=ctk.CTkFont(size=17)).grid(row=1, column=0, sticky="e", padx=8, pady=8)
        addr_e = ctk.CTkEntry(body, width=360, font=ctk.CTkFont(size=17)); addr_e.grid(row=1, column=1, padx=8, pady=8)
        def save():
            nm = name_e.get().strip()
            if not nm:
        ...
        ctk.CTkButton(body, text="Save", font=ctk.CTkFont(size=17),
                      fg_color="#2F8B2F", hover_color="#277327", command=save).grid(row=2, column=0, columnspan=2, pady=10)
    ...
    def open_patient_popup(self, pid):
        ...
        name_e = ctk.CTkEntry(info, width=360, font=ctk.CTkFont(size=17)); name_e.grid(row=0, column=1, padx=6, pady=6); name_e.insert(0, pname)
        ctk.CTkLabel(info, text="Address", font=ctk.CTkFont(size=17)).grid(row=0, column=2, sticky="e", padx=6, pady=6)
        addr_e = ctk.CTkEntry(info, width=360, font=ctk.CTkFont(size=17)); addr_e.grid(row=0, column=3, padx=6, pady=6); addr_e.insert(0, paddr)
        ctk.CTkButton(info, text="💾 Save Patient", font=ctk.CTkFont(size=17),
                      fg_color="#0B5CAB", hover_color="#084b8a",
                      command=lambda: self._save_patient(pid, name_e, addr_e, win)).grid(row=0, column=4, padx=8)
        ctk.CTkButton(info, text="🔵 Light Up Locations", font=ctk.CTkFont(size=17),
                      command=lambda: self._light_up_patient(pid, win)).grid(row=0, column=5, padx=6)
        ctk.CTkButton(info, text="🗑️ Clear All Prescriptions", font=ctk.CTkFont(size=17),
                      fg_color="#D83B01", hover_color="#B32F00",
        ...
        basket_cb = ctk.CTkComboBox(addf, values=["small","large"], width=140, font=ctk.CTkFont(size=17))
        basket_cb.set("small"); basket_cb.grid(row=0, column=5, padx=6)
        ctk.CTkButton(addf, text="➕ Add Prescription", font=ctk.CTkFont(size=17),
                      fg_color="#2F8B2F", hover_color="#277327",
                      command=lambda: self._add_prescription(pid, addr_e, med_e, qty_e, basket_cb, win, populate_rx)).grid(row=0, column=6, padx=10)
        ctk.CTkButton(addf, text="🗑️ Delete Selected", font=ctk.CTkFont(size=17),
                      fg_color="#D83B01", hover_color="#B32F00",
    ...
    def _add_prescription(self, pid, addr_e, med_e, qty_e, basket_cb, parent, refresh_cb):
        ...
        def auto_assign():
            ...
            conf = ctk.CTkToplevel(parent); conf.title("Confirm Slot"); conf.geometry("600x200")
            conf.attributes("-topmost", True)
            ctk.CTkLabel(conf, text=f"Proposed Shelf {shelf}: {pos_str}\n(Full: {', '.join(labels)})",
                         font=ctk.CTkFont(size=17)).pack(padx=12, pady=12)
            ctk.CTkButton(conf, text="✅ Confirm", font=ctk.CTkFont(size=17),
                          fg_color="#0B5CAB", hover_color="#084b8a",
                          command=lambda: self._confirm_auto(conf, pres_id, slot_ids, labels, parent, refresh_cb)).pack(side="left", padx=10, pady=10)
            ctk.CTkButton(conf, text="✋ Deny / Manual", font=ctk.CTkFont(size=17),
                          fg_color="#6B6B6B", hover_color="#585858",
        ...
        if fam:
            ...
            fp = ctk.CTkToplevel(parent); fp.title("Family Match"); fp.geometry("620x200")
            fp.attributes("-topmost", True)
            ctk.CTkLabel(fp, text=f"Same address found. Existing bin at {fam_label}.\nBin together?",
                         font=ctk.CTkFont(size=17)).pack(padx=12, pady=12)
            ctk.CTkButton(fp, text="🧺 Bin Together", font=ctk.CTkFont(size=17),
                          fg_color="#7A3DB8", hover_color="#652f98",
                          command=lambda: self._bin_together(fp, pres_id, fam_slot, fam_label, parent, refresh_cb)).pack(side="left", padx=10, pady=10)
            ctk.CTkButton(fp, text="Cancel", font=ctk.CTkFont(size=17),
                          command=lambda: (fp.destroy(), auto_assign())).pack(side="right", padx=10, pady=10)
        else:
            auto_assign()
    ...
    def _manual_assign_new(self, parent, pres_id, refresh_cb):
        ...
        s_e = ctk.CTkEntry(m, width=80, font=ctk.CTkFont(size=17)); s_e.grid(row=0, column=1, padx=8, pady=8)
        ctk.CTkLabel(m, text="Row (A–Z)", font=ctk.CTkFont(size=17)).grid(row=1, column=0, padx=8, pady=8, sticky="e")
        r_e = ctk.CTkEntry(m, width=80, font=ctk.CTkFont(size=17)); r_e.grid(row=1, column=1, padx=8, pady=8)
        ...
        ctk.CTkButton(m, text="Assign", font=ctk.CTkFont(size=17),
                      fg_color="#0B5CAB", hover_color="#084b8a", command=assign).grid(row=3, column=0, columnspan=2, pady=12)

    def _edit_prescription_popup(self, patient_id, pres_id, refresh_cb):
        e = ctk.CTkToplevel(self); e.title("Edit Prescription"); e.geometry("640x520")
        ...
        qty_e = ctk.CTkEntry(body, width=160, font=ctk.CTkFont(size=17)); qty_e.grid(row=1, column=1, padx=6, pady=6); qty_e.insert(0, str(qty) if qty is not None else "")
        ctk.CTkLabel(body, text="Date (MM/DD/YYYY)", font=ctk.CTkFont(size=17)).grid(row=2, column=0, sticky="e", padx=6, pady=6)
        date_e = ctk.CTkEntry(body, width=200, font=ctk.CTkFont(size=17)); date_e.grid(row=2, column=1, padx=6, pady=6)
        if dt:
            for fmt in ("%Y-%m-%d %H:%M:%S","%Y-%m-%d"):
        ...
        b_cb = ctk.CTkComboBox(body, values=["small","large"], width=160, font=ctk.CTkFont(size=17))
        ...
        ctk.CTkButton(body, text="Save Changes", font=ctk.CTkFont(size=17),
                      fg_color="#0B5CAB", hover_color="#084b8a", command=save_edit).grid(row=5, column=0, columnspan=2, pady=12)

    # ---------------- Shelf Assignment ----------------
    def open_shelf_assignment(self):
        win = ctk.CTkToplevel(self); win.title("Shelf Assignment"); win.geometry("980x880")
        win.attributes("-topmost", True)
        head = ctk.CTkFrame(win, fg_color="#0b5cab"); head.pack(fill="x")
        ctk.CTkLabel(head, text="Shelf Assignment",
                     font=ctk.CTkFont(size=20, weight="bold")).pack(anchor="w", padx=12, pady=8)
        ...
        for i,L in enumerate(letters, start=1):
            ...
            shelf_cb = ctk.CTkComboBox(rowf, values=["","F","L","R"], width=80, font=ctk.CTkFont(size=17)); shelf_cb.set("")
            lower_e  = ctk.CTkEntry(rowf, width=160, font=ctk.CTkFont(size=17))
            upper_e  = ctk.CTkEntry(rowf, width=160, font=ctk.CTkFont(size=17))
        ...
        def save_all():
            ...
            win.destroy()
        ctk.CTkButton(foot, text="💾 Save All & Close", font=ctk.CTkFont(size=17),
                      fg_color="#0B5CAB", hover_color="#084b8a", command=save_all).pack(side="right")
    ...
    def open_overdue_tab(self):
        ...
        ctk.CTkButton(top, text="🔴 Light Up Overdue", font=ctk.CTkFont(size=17),
                      fg_color="#D83B01", hover_color="#B32F00",
        ...
        for c in cols:
            tv.heading(c, text=c)
            tv.column(c, width=240 if c!="Locations" else 520, anchor="w")
    ...
    def _open_overdue_patient_detail(self, tree):
        ...
        top = ctk.CTkFrame(win); top.pack(fill="x", padx=10, pady=8)
        ctk.CTkButton(top, text="🔴 Light Up These", font=ctk.CTkFont(size=17),
                      fg_color="#D83B01", hover_color="#B32F00",
        ...
        for c in cols:
            tv.heading(c, text=c)
            tv.column(c, width=260 if c!="Location" else 300, anchor="w")
        tv.pack(side="left", fill="both", expand=True)
        sb = ttk.Scrollbar(tablef, orient="vertical", command=tv.yview)
        sb.pack(side="right", fill="y")
        tv.configure(yscroll=sb.set)
    ...
    def open_actions_tab(self):
        ...
        for c in cols:
            tv.heading(c, text=c)
            tv.column(c, width=180 if c!="Action" else 860, anchor="w")
        tv.pack(side="left", fill="both", expand=True)
        sb = ttk.Scrollbar(tablef, orient="vertical", command=tv.yview)
        sb.pack(side="right", fill="y")
        tv.configure(yscroll=sb.set)
        ...
        ctk.CTkButton(win, text="🔄 Refresh", font=ctk.CTkFont(size=17),
